...
public class AuthController {
    ...
    @Operation(summary = "Login de usuario",
            description = "Autentica al usuario con username y password y devuelve un token JWT junto con la información del usuario (rol: ADMIN o DOCTOR).")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Login exitoso",
                    content = @Content(schema = @Schema(implementation = AuthResponse.class))),
            @ApiResponse(responseCode = "400", description = "Credenciales inválidas o error en login",
                    content = @Content(schema = @Schema(implementation = String.class)))
    })
    @PostMapping("/login")
    public ResponseEntity<?> login(@io.swagger.v3.oas.annotations.parameters.RequestBody(
            description = "Datos de login con username y password",
            required = true,
    ...
    @Operation(summary = "Verificar token JWT",
            description = "Endpoint protegido para probar si el token JWT enviado en la cabecera es válido. Retorna 'permitido' si el token es correcto.")
    @ApiResponses(value = {
